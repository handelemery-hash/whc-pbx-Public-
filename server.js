...
// Birthdays + Procedure Reminders + Webhook write-back + Calendar→Sheet Sync
// -------------------------------------------------------------------------
...
const PHYSICIAN_DISPLAY = {
  dr_emery: "Dr Emery", dr_thompson: "Dr Thompson", dr_dowding: "Dr Dowding",
  dr_blair: "Dr Blair", dr_williams: "Dr Williams", dr_wright: "Dr Wright", dr_dixon: "Dr Dixon",
};
...
// --------------- Calendar helpers ----------
function normalizePhys(s){ return String(s||"").trim().toLowerCase().replace(/\s+/g,"_"); }
...
const Calendar = {
  async createEvent({ physician, start, end, summary, phone, note }) {
    ...
    const description = [ phone ? `Phone: ${phone}` : null, note ? `Note: ${note}` : null ].filter(Boolean).join("\n");
  ...
  async upcoming(physician, max=10){
    ...
    const { data } = await calendar.events.list({
      ...
      maxResults: Math.min(Math.max(+max||10,1),50), singleEvents:true, orderBy:"startTime"
    });
...
async function sendVoicemailEmail({ branch, caller, recordingUrl, transcript }){
  ...
  if (!transporter) { console.warn("[Email] SMTP not configured"); return; }
  const html = `
...
app.get("/calendar/upcoming/:physician", async (req,res)=>{
  try{
    const { physician } = req.params; const { max } = req.query;
    const { key, items } = await Calendar.upcoming(physician, Number(max||10));
    res.json({ ok:true, physician:PHYSICIAN_DISPLAY[key]||key, count:items.length,
      events: items.map(ev=>({ id:ev.id, summary:ev.summary, start:ev.start?.dateTime||ev.start?.date, end:ev.end?.dateTime||ev.end?.date, link:ev.htmlLink })) });
  }catch(err){ console.error("[/calendar/upcoming] error:", err?.response?.data || err.message); res.status(500).json({ ok:false, error:err.message });}
});
app.post("/calendar/delete", async (req,res)=>{
  try{ const { physician, eventId } = req.body||{};
    if(!physician||!eventId) return res.status(400).json({ ok:false, error:"physician and eventId required" });
    await Calendar.deleteEvent(physician, eventId); res.json({ ok:true });
  }catch(err){ console.error("[/calendar/delete] error:", err?.response?.data || err.message); res.status(500).json({ ok:false, error:err.message });}
});

// --------------- Retell in-call -------------
app.post("/retell/action", async (req,res)=>{
  try{
    const e = req.body || {}; const action = String(e.action||"").toLowerCase();
  ...
  }catch(err){ console.error("[/retell/action] error:", err?.response?.data || err.message); res.json({ ok:false, response:"Error." }); }
});

// --------------- Telnyx PBX -----------------
const TELNYX_API_KEY = process.env.TELNYX_API_KEY;
const TELNYX_OUTBOUND_CALLER_ID = process.env.TELNYX_OUTBOUND_CALLER_ID;
const telnyx = TELNYX_API_KEY ? axios.create({ baseURL:"https://api.telnyx.com/v2/", headers:{ Authorization:`Bearer ${TELNYX_API_KEY}` } }) : null;
async function tx(cmd, payload){ if(!telnyx) throw new Error("Telnyx not configured"); return telnyx.post(`call_commands/${cmd}`, payload); }
...
function isOfficeOpen(branchKey, when=nowTz()){
  const dow = when.day(); const minutes = when.hour()*60 + when.minute();
  if (dow===0) return false; // Sunday
  if (branchKey==="portmore"){ if (dow>=1 && dow<=5) return minutes>=600 && minutes<1020; if (dow===6) return minutes>=600 && minutes<840; return false; }
...
app.post("/telnyx/inbound", async (req,res)=>{
  ...
  try{
    const data=req.body?.data||{}, eventType=data?.event_type, payload=data?.payload||{}, callControlId=payload.call_control_id;
    ...
    if (eventType==="call.bridged"){ await tx("playback_stop",{ call_control_id:callControlId }); return res.json({ ok:true }); }
    ...
    res.json({ ok:true });
  }catch(err){ console.error("[/telnyx/inbound] error:", err?.response?.data || err.message); res.status(200).json({ ok:true }); }
});

// --------------- Jobs scaffolding -----------
function assertToken(req,res){ const t=String(req.query.token||""); if(!STATUS_TOKEN||t!==STATUS_TOKEN){ res.status(401).json({ ok:false, error:"unauthorized" }); return false;} return true; }
app.post("/jobs/run-reminders", async (req,res)=>{ if(!assertToken(req,res))return; try{ return res.json({ ok:true, results:[] }); }catch(err){ console.error("[/jobs/run-reminders] error:", err?.response?.data || err.message); return res.status(200).json({ ok:false, error:err.message }); }});
...
app.post("/jobs/run-birthdays", async (req,res)=>{
  ...
  try{
    ...
    const read = await sheets.spreadsheets.values.get({ spreadsheetId:SHEET_ID, range:RANGE, valueRenderOption:"UNFORMATTED_VALUE", dateTimeRenderOption:"FORMATTED_STRING" });
    const rows = read.data.values || []; if (!rows.length) return res.json({ ok:true, processed:0, details:[] });

    const headers = (rows[0]||[]).map(h=>String(h||"").trim().toLowerCase());
    const col = (n)=> headers.indexOf(String(n).trim().toLowerCase());
    const idx = {
      full_name: col("full_name"), phone: col("phone_e164"), dob: col("dob_yyyy_mm_dd"), branch: col("branch"),
      opt_out: col("opt_out"), opt_out_reason: col("opt_out_reason"), last_called_year: col("last_called_year"),
      last_outcome: col("last_outcome"), last_outcome_ts: col("last_outcome_ts"),
      deferred_for: col("deferred_for_yyyy_mm_dd"), deferred_reason: col("deferred_reason"),
      dob_correction: col("dob_correction"), new_phone_candidate: col("new_phone_candidate"),
      status_flag: col("status_flag"), status_note: col("status_note"),
      preferred_contact: col("preferred_contact"), caregiver_name: col("caregiver_name"), caregiver_phone: col("caregiver_phone"),
      pause_until: col("pause_until_yyyy_mm_dd"), last_call_id: col("last_call_id"), retry_count: col("retry_count")
    };
    const val=(r,i)=> (i>=0 && r[i]!=null)? String(r[i]).trim() : "";
    const asBool=(s)=> /^true|1|yes|y$/i.test(String(s||"").trim());
    const parseYmd=(s)=>{ const t=String(s||"").trim(); if(!t) return null; const m=dayjs(t,["YYYY-MM-DD","YYYY/M/D","YYYY/M/DD","YYYY/MM/D","YYYY/MM/DD"],true); return m.isValid()?m:null; };

    const today=nowTz(), todayY=today.year(), todayMD=today.format("MM-DD"), insideWindow=insideWindowNow();
    ...
    const indexToLetters=(n)=>{ let s=""; n++; while(n>0){ const rem=(n-1)%26; s=String.fromCharCode(65+rem)+s; n=Math.floor((n-1)/26);} return s; };
    const startBase=letterToIndex(startColLetter.toUpperCase());
    const setCell=(row1,col0,val_)=>{ const a1=`${indexToLetters(startBase+col0)}${row1}`; updates.push({ range:`${RANGE.split("!")[0]}!${a1}`, values:[[val_]] }); };

    const offs={ last_called_year: idx.last_called_year, last_outcome: idx.last_outcome, last_outcome_ts: idx.last_outcome_ts, last_call_id: idx.last_call_id };

    for (let r=1; r<rows.length; r++){
      const row=rows[r]||[]; const row1=r+1;
      const fullName=val(row,idx.full_name); const phone=val(row,idx.phone);
      const dobStr=val(row,idx.dob); const branch=(val(row,idx.branch).toLowerCase()||"winchester");
      const optOut=asBool(val(row,idx.opt_out)); const statusFlag=val(row,idx.status_flag).toUpperCase();
      const pauseUntil = parseYmd(val(row,idx.pause_until)); const deferredFor=parseYmd(val(row,idx.deferred_for));
      const dobCorrection=parseYmd(val(row,idx.dob_correction)); const lastCalledYear=val(row,idx.last_called_year);

      const dob = dobCorrection || parseYmd(dobStr); if(!dob||!fullName) continue;
      if (dob.tz(LOCAL_TZ).format("MM-DD") !== todayMD) continue;

      const writeOutcome=(k)=>{ if(offs.last_outcome>=0) setCell(row1,offs.last_outcome,k); if(offs.last_outcome_ts>=0) setCell(row1,offs.last_outcome_ts,isoLocal()); };
      const writeYearNow=()=>{ if(offs.last_called_year>=0) setCell(row1,offs.last_called_year,String(todayY)); };
      const writeCallId=(id)=>{ if(offs.last_call_id>=0 && id) setCell(row1,offs.last_call_id,id); };
      ...
      if (ENABLE_BDAY_DIAL && RETELL_API_KEY && RETELL_OUTBOUND_AGENT_ID && RETELL_OUTBOUND_FROM){
        try{
          ...
          const dial = await placeRetellCall({ to:phone, from:RETELL_OUTBOUND_FROM, agent_id:RETELL_OUTBOUND_AGENT_ID, variables, metadata:meta, webhook_url:webhookUrl });
          writeYearNow(); writeOutcome("call_placed"); writeCallId(dial?.call_id || dial?.id || "");
    ...
    return res.json({ ok:true });
  }catch(err){ console.error("[/jobs/run-birthdays] error:", err?.response?.data || err.message); return res.status(200).json({ ok:false, error:err.message }); }
});
...
app.post("/jobs/run-procedure-reminders", async (req,res)=>{
  ...
  try{
    ...
    const read = await sheets.spreadsheets.values.get({ spreadsheetId:SHEET_ID, range:RANGE, valueRenderOption:"UNFORMATTED_VALUE", dateTimeRenderOption:"FORMATTED_STRING" });
    const rows = read.data.values || []; if (!rows.length) return res.json({ ok:true, processed:0 });
    ...
    const val=(r,i)=> (i>=0 && r[i]!=null)? String(r[i]).trim() : "";
    const asBool=(s)=> /^true|1|yes|y$/i.test(String(s||"").trim());
    const parseYmd=(s)=>{ const t=String(s||"").trim(); if(!t) return null; const m=dayjs(t,["YYYY-MM-DD","YYYY/M/D","YYYY/M/DD","YYYY/MM/D","YYYY/MM/DD"],true).tz(LOCAL_TZ); return m.isValid()?m:null; };

    const offsets = (process.env.PROC_REMINDER_OFFSETS || "7,3,1").split(",").map(s=>parseInt(s.trim(),10)).filter(n=>Number.isFinite(n)&&n>=0);
    ...
    const startBase=letterToIndex(startColLetter.toUpperCase());
    const updates=[];
    const setCell=(row1,col0,val_)=>{ if(col0<0)return; const a1=`${indexToLetters(startBase+col0)}${row1}`; updates.push({ range:`${RANGE.split("!")[0]}!${a1}`, values:[[val_]] }); };

    for (let r=1; r<rows.length; r++){
      const row=rows[r]||[]; const row1=r+1;
      const fullName=val(row,idx.full_name), phone=val(row,idx.phone), branch=(val(row,idx.branch).toLowerCase()||"winchester");
      const optOut=asBool(val(row,idx.opt_out)); const statusFlag=val(row,idx.status_flag).toUpperCase();
      const pauseUntil=parseYmd(val(row,idx.pause_until));
      const apptIso=val(row,idx.appt_iso);
      let appt=null;
      if (apptIso) { const m=parseLocalIso(apptIso); appt=m?.isValid()?m:null; }
      else {
        const d=parseYmd(val(row,idx.appt_date));
        if (d) {
          const t = val(row,idx.appt_time);
          if (t && /^\d{1,2}:\d{2}$/.test(t)){ const [H,M]=t.split(":").map(Number); appt=d.hour(H).minute(M); }
      ...
      if (optOut){ setCell(row1,idx.last_outcome,"skipped_opt_out"); continue; }
      if (statusFlag==="DO_NOT_CALL" || statusFlag==="INACTIVE"){ setCell(row1,idx.last_outcome,`skipped_${statusFlag.toLowerCase()}`); continue; }
      if (pauseUntil && pauseUntil.isAfter(today,"day")){ setCell(row1,idx.last_outcome,"paused_until"); continue; }
      if (!insideWindow){ setCell(row1,idx.last_outcome,"skipped_outside_window"); continue; }
    ...
    return res.json({ ok:true });
  }catch(err){ console.error("[/jobs/run-procedure-reminders] error:", err?.response?.data || err.message); return res.status(200).json({ ok:false, error:err.message }); }
});
...
function nextRetryDate(base=nowTz()){
  ...
  if (RETRY_SKIP_WEEKENDS){ while (d.day()===0 || d.day()===6) d = d.add(1,"day"); }
  return d;
}

// --------------- Retell Webhook w/ Retries -
app.post("/retell/outbound/callback", async (req,res)=>{
  const token=String(req.query.token||""); if(!STATUS_TOKEN || token!==STATUS_TOKEN) return res.status(401).json({ ok:false });
  try{
    const body=req.body||{}; const callId=body.call_id||body.id||""; const final=(body.final_status||body.status||"").toLowerCase(); const meta=body.metadata||{};
    const SHEET_ID=meta.sheet_id||""; const RANGE=meta.range||"Sheet1!A:Z"; const row1=Number(meta.row_number||0); const kind=String(meta.kind||"").toLowerCase();
    ...
    const read = await sheets.spreadsheets.values.get({ spreadsheetId:SHEET_ID, range:RANGE, valueRenderOption:"UNFORMATTED_VALUE", dateTimeRenderOption:"FORMATTED_STRING" });
    const rows=read.data.values||[]; if(!rows.length) return res.json({ ok:false, error:"empty sheet" });
    ...
    const startBase=letterToIndex(startColLetter.toUpperCase());
    const updates=[];
    const setCell=(row1_,col0,val_)=>{ if(col0<0)return; const a1=`${indexToLetters(startBase+col0)}${row1_}`; updates.push({ range:`${RANGE.split("!")[0]}!${a1}`, values:[[val_]] }); };
    const getCell=(row1_,col0)=>{ if(col0<0) return ""; const row=rows[row1_-1]||[]; return (row[col0]!=null)? String(row[col0]).trim():""; };
    ...
    else if (final.includes("complete")) outcome=(kind==="birthday")?"wished_happy_birthday":"reminder_completed";

    setCell(row1, idx.last_outcome, outcome);
    setCell(row1, idx.last_outcome_ts, isoLocal());
    if (kind==="birthday" && idx.last_called_year>=0) setCell(row1, idx.last_called_year, String(nowTz().year()));
    if (idx.last_call_id>=0 && callId) setCell(row1, idx.last_call_id, callId);

    // Retry logic
    if (RETRY_ENABLE && RETRY_OUTCOMES.includes(outcome)){
      let currentRetries=0; if (idx.retry_count>=0){ const cur=parseInt(getCell(row1,idx.retry_count),10); if(Number.isFinite(cur)) currentRetries=cur; }
    ...
    return res.json({ ok:true });
  }catch(err){ console.error("[/retell/outbound/callback] error:", err?.response?.data || err.message); return res.status(200).json({ ok:false, error:err.message }); }
});

// --------------- Calendar → Procedures Sheet Sync ----------
/**
 * For each branch procedure calendar:
 * - Pull events in lookback..lookahead window
 * - Infer service from summary; parse patient/contact from description if present
 * - Upsert into Procedures sheet by event_id
 */
app.post("/jobs/sync-procedures", async (req,res)=>{
  ...
  try{
    ...
    const idx={
      ...
      appt_date: colIndex("appt_yyyy_mm_dd"),
      appt_time: colIndex("appt_time_hh_mm"),
    ...
    if (!haveHeaders) return res.json({ ok:false, error:"Procedure sheet missing required headers: event_id, full_name, phone_e164, appt_iso, branch, service" });

    // Map existing by event_id
    const existing = new Map();
    for (let r=1; r<rows.length; r++){
      const rid = rows[r]?.[idx.event_id];
      if (rid) existing.set(String(rid).trim(), r+1); // store 1-based row
    }

    const appends = []; // rows to append
    const updates = []; // {row1, col, value}

    // Helper to infer service from summary
    ...
    // Parse from description "Phone:" and "Name:" if staff typed it
    const parseDesc=(desc)=>{
      const out={ name:"", phone:"" };
      const d=String(desc||"");
      const m1=d.match(/name\s*:\s*(.+)/i); if(m1) out.name=m1[1].trim();
      const m2=d.match(/phone\s*:\s*([+0-9\-\s]+)/i); if(m2) out.phone=m2[1].replace(/\s+/g,"").replace(/-/g,"");
      return out;
    };

    // Pull events for each branch calendar
    for (const [branch, calId] of Object.entries(PROCEDURE_CALENDARS)){
      const { data } = await calendar.events.list({
        calendarId: calId, timeMin, timeMax, singleEvents:true, orderBy:"startTime", maxResults:2500
      });
      ...
      for (const ev of items){
        ...
        if (row1){ // update minimal fields if blank
          const row = rows[row1-1] || [];
          const setIfEmpty=(colName, val)=>{
            const c = colIndex(colName); if (c<0) return;
            const cur = row[c]; if (!cur && val) updates.push({ row1, col:c, value:val });
          };
          ...
          // append full row aligned to headers
    ...
    if (updates.length){
      const dataReq = updates.map(({row1,col,value})=>{
        ...
        const indexToLetters=(n)=>{ let s=""; n++; while(n>0){ const r=(n-1)%26; s=String.fromCharCode(65+r)+s; n=Math.floor((n-1)/26);} return s; };
        const startBase=letterToIndex(startColLetter.toUpperCase());
        const a1 = `${indexToLetters(startBase+col)}${row1}`;
        return { range:`${RANGE.split("!")[0]}!${a1}`, values:[[value]] };
      });
      await sheets.spreadsheets.values.batchUpdate({
        spreadsheetId: SHEET_ID, requestBody:{ valueInputOption:"RAW", data:dataReq }
      });
    }
    if (appends.length){
...
app.get("/status.json", (req,res)=>{
  const t=String(req.query.token||""); if(!STATUS_TOKEN||t!==STATUS_TOKEN) return res.status(401).json({ ok:false, error:"unauthorized" });
  const haveGoogle = !!(process.env.GOOGLE_APPLICATION_CREDENTIALS_JSON || process.env.GOOGLE_CREDENTIALS_B64);
  res.json({
    ok:true, time:isoLocal(), tz:LOCAL_TZ,
    checks:{
      ...
      procedures: { sheet_id_present: !!process.env.PROCEDURES_SHEET_ID, range: process.env.PROCEDURES_RANGE || "Sheet1!A:Z", offsets: (process.env.PROC_REMINDER_OFFSETS||"7,3,1") },
      sync: { look_ahead_days: Number(process.env.PROC_SYNC_LOOKAHEAD_DAYS||30), look_back_days: Number(process.env.PROC_SYNC_LOOKBACK_DAYS||1) },
      outbound: {
